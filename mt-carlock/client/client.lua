...
CreateThread(function()
    ...
    TriggerServerEvent('mt-carlock:getOwnedKeys')
end)
        ...
        if IsPedInAnyVehicle(PlayerPedId(), false) then
            ...
            if IsDisabledControlJustPressed(0, 311) then
                        ...
                        exports['mythic_progbar']:Progress({
                            ...
                            if not cancelled then
                                if not hasCarKey(plate) then
                                    TriggerServerEvent('mt-carlock:searchVehicle', plate)
                                    showHelp = true
                ...
                elseif searchedVehicles[plate] then
                    TriggerEvent('mt-carlock:notify', Language['already_searched'])
                end
            end
        end
    end
                            ...
                            if Config.MythicProgbar then
                                TriggerEvent("mythic_progbar:client:progress", {
                                    ...
                                    if not cancelled then
                                        ...
                                        TriggerEvent('mt-carlock:notify', Language['handed_keys'])
                                        TriggerServerEvent('mt-carlock:addKey', targetPlate)
                                    else
                                        nowUnlocked[targetPlate] = false
                                        TriggerEvent('mt-carlock:notify', Language['action_cancelled'])
                                    end
                                end)
                            else
                                Wait(5000)
            ...
            if DoesEntityExist(vehicle) and GetPedInVehicleSeat(vehicle, -1) == playerPed and not hotwiredVehicles[plate] and not Config.noKeysNeeded[GetDisplayNameFromVehicleModel(GetEntityModel(vehicle))] and not hasCarKey(plate) then
                ...
                if not hotwiredVehicles[plate] and failedHot[plate] ~= Config.MaxHotwireAttempts then
                    if IsDisabledControlJustPressed(0, 74) and showHelp then
                        ...
                        if skill then
                            ...
                            TriggerServerEvent('mt-carlock:addKey', plate)
                            TriggerEvent('mt-carlock:notify', Language['hotwire_success'])
                            showHelp = true
                            inCarAnimation(false)
                        else
                            showHelp = true
                            if failedHot[plate] == nil then
                                ...
                                TriggerEvent('mt-carlock:notify', Language['hotwire_failed'])
                                inCarAnimation(false)
                            end
                        end
                    end
                elseif hotwiredVehicles[plate] then
                    if IsDisabledControlJustPressed(0, 74) then
                        TriggerEvent('mt-carlock:notify', Language['already_hotwired'])
                    end
                elseif failedHot[plate] then
                    if IsDisabledControlJustPressed(0, 74) then
                        TriggerEvent('mt-carlock:notify', Language['bad_wires'])
                    end
                end
            end
        end
...
RegisterNetEvent('mt-carlock:startVehicle')
AddEventHandler('mt-carlock:startVehicle', function()
    local playerPed = PlayerPedId()
    local locVeh = GetVehiclePedIsIn(playerPed, false)
    SetVehicleEngineOn(locVeh, true, true)
end)

RegisterNetEvent('mt-carlock:syncKeys')
AddEventHandler('mt-carlock:syncKeys', function (vehKey, identifier)
    savedKeys = vehKey
    playerIdent = identifier
end)


AddEventHandler('mt-carlock:togglelocks', function()
    local playerPed = PlayerPedId()
        ...
        CreateThread(function()
            if hasCarKey(GetVehicleNumberPlateText(vehicle)) then
                ...
                if GetVehicleDoorLockStatus(vehicle) == 1 then
                    ...
                    TriggerEvent('mt-carlock:notify', Language['vehicle_locked'])
                elseif GetVehicleDoorLockStatus(vehicle) == 2 then
                    SetVehicleDoorsLocked(vehicle, 1)
                    if Config.CustomSounds then
                        TriggerServerEvent("InteractSound_SV:PlayWithinDistance", 8, "unlock", 0.3)
                    end
                    TriggerEvent('mt-carlock:notify', Language['vehicle_unlocked'])
                end
            end
        end)
    else
        TriggerEvent('mt-carlock:notify', Language['no_vehiclefound'])
    end
end)

RegisterCommand('toggleLocks', function()
    TriggerEvent('mt-carlock:togglelocks')
end)
...
RegisterCommand(Config.GiveCommand, function()
    ...
    if not hasCarKey(GetVehicleNumberPlateText(closeVeh)) then
        TriggerEvent('mt-carlock:notify', Language['no_keys'])
        return
    end
    if #(GetEntityCoords(closeVeh) - GetEntityCoords(playerPed)) > 5 then
        TriggerEvent('mt-carlock:notify', Language['too_far'])
        return
    end
    local target, dist = ESX.Game.GetClosestPlayer()
    if dist ~= -1 and dist < 10 then
        TriggerServerEvent('mt-carlock:giveKey', GetPlayerServerId(target), GetVehicleNumberPlateText(closeVeh))
    else
        TriggerEvent('mt-carlock:notify', Language['no_player_nearby'])
    end
end)

RegisterNetEvent('mt-carlock:lockpick')
AddEventHandler('mt-carlock:lockpick', function()
    local playerPed = PlayerPedId()
        ...
        if not IsPedInAnyVehicle(playerPed, false) then
            if GetVehicleDoorLockStatus(closeVeh) ~= 1 then
                ...
                if skill then
                    ...
                    TriggerEvent('mt-carlock:notify', Language['lockpick_success'])
                    ...
                    TriggerServerEvent('mt-carlock:removepick')
                else
                    inLockpickAnim(false)
                    TriggerEvent('mt-carlock:notify', Language['lockpick_unsuccessful'])
                    TriggerServerEvent('mt-carlock:removepick')
                end
            end
        end
...
RegisterNetEvent('mt-carlock:addKeys')
AddEventHandler('mt-carlock:addKeys', function(plates)
    TriggerServerEvent('mt-carlock:addKeysOwned', plates)
end)
...
exports('RemoveKey', function(plate)
    ...
    if savedKeys[plate] ~= nil then
        savedKeys[plate] = nil
        -- Optional: Sync with server if needed, but usually carlock scripts handle this client-side for the current session
    end
end)
