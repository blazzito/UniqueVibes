...
client.updatePlayerAppearance = updatePlayerAppearance
...
local function setCamera(key)
    ...
    if cameraHandle then
        ...
        CreateThread(function()
            ...
            if not DoesCamExist(cameraHandle) then return end
...
function client.startPlayerCustomization(cb, conf)
    ...
    TriggerEvent("illenium-appearance:client:onMenuToggle", true)
    pcall(function() exports['mt-textui']:SetGlobalSuppression(true) end)
end
...
function client.exitPlayerCustomization(appearance)
    ...
    client.cleanUpPlayerState(cache.ped, cache.playerId)

    SendNuiMessage(json.encode({
        type = "appearance_hide",
        payload = {}
    }))
    TriggerEvent("illenium-appearance:client:onMenuToggle", false)
    ...
    pcall(function() exports['mt-textui']:SetGlobalSuppression(false) end)
end
