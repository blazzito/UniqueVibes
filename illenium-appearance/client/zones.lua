...
local function onStoreEnter(data)
    ...
    if jobName == (store.job or store.gang) or (not store.job and not store.gang) then
        ...
        exports['mt-notify']:ShowDrawText3D(currentZone.interactionId)
...
local function onZoneInside(self)
    ...
    if newCoords then
                                exports['mt-notify']:Update3DTextCoords("store_interaction_" .. store.id, newCoords + vector3(0, 0, 0.4))
    end
end
...
local function onZoneExit()
    if currentZone and currentZone.interactionId then
        exports['mt-notify']:HideDrawText3D(currentZone.interactionId)
    end
    currentZone = nil
...
local function SetupZone(store, onEnter, onExit)
    ...
    local hasValidPoints = points and #points >= 3
    if (Config.UseRadialMenu or store.usePoly) and hasValidPoints then
        local zoneId = store.id and ("store_interaction_" .. store.id) or (store.name == "clothingRoom" and ("store_interaction_clothingRoom_" .. store.index)) or (store.name == "playerOutfitRoom" and ("store_interaction_playerOutfitRoom_" .. store.index))
        
        local zone = lib.zones.poly({
            points = points,
            thickness = size.z, 
            debug = DebugZonesActive or Config.Debug,
            onEnter = onEnter,
            onExit = onExit,
            inside = onZoneInside,
            interactionId = zoneId
        })

        if zoneId then
            exports['mt-notify']:create3DTextUI(zoneId, {
                type = "textui",
                coords = textCoords,
                text = message,
                key = "E",
                ...
                zone = zone, -- Attach zone for automatic visibility logic
                displayDist = 99.0, -- Visibility controlled by zone
                interactDist = 99.0, -- Always interactable if inside zone
            })
            exports['mt-notify']:HideDrawText3D(zoneId) -- Start hidden
        end
...
local function SetupBlips()
    ...
    for i, store in ipairs(CachedStores) do
        ...
        if not blipConfig then goto continue end

        local showBlip = false
        if store.showBlip ~= nil then
            showBlip = (store.showBlip == true or store.showBlip == 1 or store.showBlip == "true")
        else
            showBlip = blipConfig.Show
        end

        if showBlip then
            local blipType = blipConfig.Sprite or 73
            local blipColor = blipConfig.Color or 47
            local blipName = blipConfig.Name or "Store"
...
local function SetupStoreZones()
    ...
    -- Explicitly remove old mt-notify nodes for ALL stores in Zones.Store before rebuilding
    for i = 1, #Zones.Store do
        ...
        if CachedStores[i] and CachedStores[i].id then
            exports['mt-notify']:DeleteText3D("store_interaction_" .. CachedStores[i].id)
        end
...
CreateThread(function()
    ...
    if not Config.UseRadialMenu then
        -- Logic is now handled by mt-notify events, no loop needed
    end
end)
...
RegisterNetEvent("illenium-appearance:client:setDebugZones", function(enabled)
    DebugZonesActive = enabled
    SetupZones()
end)
