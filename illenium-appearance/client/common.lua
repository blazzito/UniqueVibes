function ShowNotification(title, description, type, length)
    lib.notify({
        title = title,
        description = description,
        type = type,
        duration = length or Config.NotifyOptions.duration
    })
end
...
function OpenBarberShop(customCost)
    local config = GetDefaultConfig()
    config.headOverlays = true
    OpenShop(config, false, "barber", customCost)
end

function OpenTattooShop(customCost)
    local config = GetDefaultConfig()
    config.tattoos = true
    OpenShop(config, false, "tattoo", customCost)
end

function OpenSurgeonShop(customCost)
    local config = GetDefaultConfig()
    config.headBlend = true
    config.faceFeatures = true
    OpenShop(config, false, "surgeon", customCost)
end
