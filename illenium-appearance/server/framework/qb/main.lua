...
function Framework.CheckPlayerHasPermission(src, permission)
    if permission == "admin" or permission == "superadmin" then
        return QBCore.Functions.HasPermission(src, permission) or (IsPlayerAceAllowed and IsPlayerAceAllowed(src, "command"))
    end
    return IsPlayerAceAllowed and IsPlayerAceAllowed(src, permission)
end
