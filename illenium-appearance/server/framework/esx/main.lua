...
function Framework.SaveAppearance(appearance, citizenID)
    ...
    return Database.Users.UpdateSkinForUser(citizenID, json.encode(appearance))
end
...
function Framework.CheckPlayerHasPermission(src, permission)
    local Player = ESX.GetPlayerFromId(src)
    if not Player then return false end
    
    -- Check ESX group
    local group = Player.getGroup()
    if group == "admin" or group == "superadmin" or group == permission then
        return true
    end

    -- Check ACE
    if IsPlayerAceAllowed and IsPlayerAceAllowed(src, permission) then
        return true
    end
