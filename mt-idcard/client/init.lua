...
--- Registering callback for internal mugshot generation
callback.register('bl_idcard:getMugShot', function()
...
callback.register('bl_idcard:use', function(itemName)
    ...
    if not target then
        lib.notify({
            title = "Nadie cerca",
            description = "Mira a quién le quieres mostrar el documento.",
            type = 'error'
        })
