...
local function getLicenseData(source, licenseType, mugshot)
    ...
    if type(imageURL) ~= 'string' or imageURL == "" then
        imageURL = sex == 'm' and "https://img.icons8.com/color/120/a9a9a9/circled-user-male-skin-type-1-2.png" or "https://img.icons8.com/color/120/a9a9a9/circled-user-female-skin-type-1-2.png"
    elseif not imageURL:find('data:image') and not imageURL:find('http') then
    ...
    -- Si el framework no cargó la nacionalidad, la buscamos directamente en la DB
    if not nationality then
        nationality = MySQL.scalar.await('SELECT nationality FROM users WHERE identifier = ?', { xPlayer.identifier })
    end
