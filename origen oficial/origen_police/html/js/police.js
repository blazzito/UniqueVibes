policeFunctions = {
						...
						note.parent()
                            ...
                            <li class="list-group-item list-group-item-action scale-in" note-id="${cb.id}">
                                ...
                                <div class="note-info d-flex">
                                    ...
                                    <div class="note-date"><i class="fa-solid fa-calendar-days"></i> ${date.date}</div>
	...
	toggleRobberyRequestsCard: function (visible) {
		$('.police .btn-robbery-requests').toggle(visible === true);
	},
	...
	alternarServicio: function (cb) {
		...
		fetch('LoadPolicePage', { page: "home" }).done((cb) => {
			...
			policeFunctions.toggleRobberyRequestsCard(cb.robberyRequestsVisible);
		});
	},
			...
			if (cb) {
					...
					cb.notes.map(function (note) {
						...
						if (note.fixed) {
                            ...
                            <li class="list-group-item list-group-item-action pinned" note-id="${note.id}">
                                ...
                                <div class="note-info d-flex">
                                    <div class="note-author"><i class="fa-solid fa-user"></i> ${note.author}</div>
                                    <div class="note-date"><i class="fa-solid fa-calendar-days"></i> ${date.date}</div>
                                    <div class="note-hour"><i class="fa-solid fa-clock"></i> ${date.time}</div>
                                </div>
                                <div class="delete-button">
                                    <i class="fa-solid fa-trash"></i>
                                </div>
                ...
                <div class="row mt-2">
                    ...
                    <div class="col-6 pe-1">
                        <div class="info-box m-1 mt-2">
                            <div class="d-flex justify-content-between align-items-center">
                                <h4><i class="fa-solid fa-note-sticky"></i> ${Translations.Reports}</h4>
                            </div>
