...
settingsFunctions = {
                            ...
                            <div class="d-flex w-100">
                                <div class="w-50">
                                    <div class="station-name">
                                        <i class="fa-solid fa-building-shield" aria-hidden="true"></i>${station.label}
                                    </div>
                                </div>
                                <div class="w-50">
    ...
    setActiveState: function(state) {
        ...
        TriggerCallback('origen_police:callback:SetActiveStation', {station: currentStation, active: state}).done((cb) => {
            if(cb) {
                ...
                // Actualizar la card en la lista sin tener que reiniciar
                const $card = $(`.app.police .setting-container[tab*='station'] .station-list .station[station="${currentStation}"]`);
                if($card.length) {
                    // Usar attr para manejar el !important del inline style original
                    $card.attr("style", state ? "" : "opacity:0.7 !important;");
                }
            }
        });
    },
    ...
    createMaker: function(markerName, multiple) {
        const stationToRefresh = currentStation;
        fetch("CreateMarker", {station: currentStation, markerName: markerName, multiple: multiple}).then((data) => {
            if(data) {
                closeMenu();
                // Refrescar el detalle de la station para mostrar el marker recién creado
                if(stationToRefresh) {
                    setTimeout(() => {
                        settingsFunctions.loadStation(stationToRefresh);
                    }, 300);
                }
            }
        });
    ...
    openCreateStationModal: function() {
        OpenModal(
            "New Station",
            `<div style="display:flex;flex-direction:column;gap:1.2vh;margin-top:1.5vh;">
                <div>
                    <label style="font-size:1.2vh;font-weight:600;margin-bottom:0.4vh;display:block;">Display label</label>
                    <input id="new-station-label" type="text" class="form-control" placeholder="Mission Row 2" maxlength="60"
                        style="background:rgba(255,255,255,0.07);border:1px solid rgba(255,255,255,0.15);color:#fff;border-radius:8px;padding:0.8vh 1.2vh;font-size:1.3vh;width:100%;">
                </div>
                <p id="new-station-error" style="color:#e05c5c;font-size:1.1vh;margin:0;min-height:1.5vh;"></p>
            </div>`,
            `<button class="btn btn-primary" style="font-size:1.2vh;padding:0.6vh 2vh;" onclick="settingsFunctions.confirmCreateStation()">Create</button>`,
            Translations.Cancel
        );
        setTimeout(() => { $("#new-station-label").focus(); }, 150);
    },

    confirmCreateStation: function() {
        const label = $("#new-station-label").val().trim();

        if(!label) {
            $("#new-station-error").text("Display label is required.");
            return;
        }

        TriggerCallback('origen_police:callback:CreateStation', { label }).done((cb) => {
            if(!cb) return;
            if(!cb.ok) {
                const msgs = {
                    label_empty:   "Display label is required.",
                    name_invalid:  "Name contains invalid characters.",
                    name_exists:   "A station with that name already exists.",
                };
                $("#new-station-error").text(msgs[cb.error] || "Unknown error.");
                return;
            }
            CloseModal();
            sendNotification('success', "Station created", `Station "${label}" was created successfully.`);
            // Recargar la lista y abrir la station recién creada
            settingsFunctions.loadStations();
            setTimeout(() => {
                settingsFunctions.loadStation(cb.name);
            }, 400);
        });
    },
}
