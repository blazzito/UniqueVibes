...
window.addEventListener('message', function (event) {
	...
	} else if (event.data.action == 'StationMarkerCreated') {
		// Refrescar el panel de detalle de la station cuando se crea un marker nuevo
		if(typeof settingsFunctions !== 'undefined' && event.data.station) {
			settingsFunctions.loadStation(event.data.station);
		}
			...
			event.data.tabs.forEach(value => {
				if(!value.hidden) {
					$(".friends .police-tab-list > .d-flex").before(`
						<div class="police-tab ${value.defaultSelected  ? 'selected' : ''}" tab="${value.index}">
			...
			event.data.objects.forEach(value => {
				if(!value.hidden) {
					$(".interaccion-list .row").append(`
						<div class="col-4 p-1">
							<div class="com-item flex-column action-police ${value.customClass || ''}" command="${value.customEvent ? value.customEvent : `plceobj ${value.name}`}" ${value.dontClose ? 'dontClose' : ''}>
					...
					if(!item.hidden) {
						if(item.header) {
							...
							$lastContainer.append(`
								<div class="action-police" ${actionAttr} ${item.dontClose ? 'dontClose' : ''}>
									<i class="${item.icon}"></i> 
