function SearchCitizen(citizenid, job)
    if Config.Framework == "qbcore" then
        return MySQL.awaitQuery([[
            SELECT citizenid, charinfo, image
            FROM players
            WHERE LOWER(citizenid) LIKE LOWER(@text)
            OR LOWER(CONCAT(
        ...
        return MySQL.awaitQuery([[
            SELECT identifier, firstname, lastname, image
            FROM users
            WHERE LOWER(identifier) LIKE LOWER(@text)
            OR LOWER(CONCAT(firstname, " ", lastname)) LIKE LOWER(@text)
        ]] .. job .. [[
            LIMIT 50
        ]], {
            ["@text"] = "%" .. citizenid .. "%"
