...
function FW_SpawnVehicle(model, cb, coords, isNetwork)
    local spawnCoords = type(coords) == "table" and (coords.x and vector3(coords.x, coords.y, coords.z) or vector3(coords[1], coords[2], coords[3])) or (type(coords) == "vector4" and vector3(coords.x, coords.y, coords.z) or coords)
    local heading = type(coords) == "table" and (coords.w or coords.h or coords[4] or 0.0) or (type(coords) == "vector4" and coords.w or 0.0)
    
    Framework.Game.SpawnVehicle(model, spawnCoords, heading, function(veh)
