...
Framework.Functions.GetPlayersOnDuty = function(jobNameOrCategory)
    local players = {}

    if not jobNameOrCategory then
        return players, 0
    end

    local jobCategory = Config.JobCategory[jobNameOrCategory] and jobNameOrCategory or GetJobCategory(jobNameOrCategory)

    if not jobCategory or type(GetPlayersInDuty) ~= "function" then
        return players, 0
    end

    local dutyPlayers = GetPlayersInDuty(jobCategory)
    local shouldFilterByJob = not Config.JobCategory[jobNameOrCategory]

    for i = 1, #dutyPlayers do
        local playerId = dutyPlayers[i]
        local xPlayer = Framework.GetPlayerFromId(playerId)

        if xPlayer and (not shouldFilterByJob or xPlayer.job.name == jobNameOrCategory) then
