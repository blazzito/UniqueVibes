...
local RADIO_ANIM_EXTERNAL = true
...
RegisterCommand('+radiotalk', function()
	...
	if not radioPressed then
		if radioChannel > 0 then
			...
			if shouldPlayAnimation and not RADIO_ANIM_EXTERNAL then
			...
			CreateThread(function()
				...
				while radioPressed do
					...
					if shouldPlayAnimation and not RADIO_ANIM_EXTERNAL and HasAnimDictLoaded(dict) then
...
RegisterCommand('-radiotalk', function()
	if radioChannel > 0 and radioPressed then
		...
		playMicClicks(false)
		if GetConvarInt('voice_enableRadioAnim', 1) == 1 and not RADIO_ANIM_EXTERNAL then
