...
function InitAppearance()
    ...
    lib.callback("illenium-appearance:server:getAppearance", false, function(appearance)
        ...
        -- Fix for combat bug: Clear all states AFTER the skin is loaded
        client.cleanUpPlayerState()
    end)
    ResetBlips()
    if Config.BossManagedOutfits then
...
function OpenShop(config, isPedMenu, shopType, customCost)
    CurrentShopType = shopType
    CurrentShopCost = customCost

    local function startCustomization()
        client.startPlayerCustomization(function(appearance)
            if appearance then
                if shopType ~= "tattoo" then
                    TriggerServerEvent("illenium-appearance:server:chargeCustomer", shopType, customCost)
                end
                TriggerServerEvent("illenium-appearance:server:saveAppearance", appearance)
            else
                ShowNotification(_L("cancelled.title"), _L("cancelled.description"), "inform")
            end
            Framework.CachePed()
        end, config)
    end

    if shopType == "tattoo" or isPedMenu then
        startCustomization()
    else
        lib.callback("illenium-appearance:server:hasMoney", false, function(hasMoney, money)
            if not hasMoney then
                ShowNotification("Cannot Enter Shop", "Not enough cash. Need $" .. money, "error")
                return
            end
            startCustomization()
        end, shopType, customCost)
    end
end

local function OpenClothingShop(isPedMenu, customCost)
    local config = GetDefaultConfig()
    ...
    OpenShop(config, isPedMenu, "clothing", customCost)
end
...
RegisterNetEvent("illenium-appearance:client:importOutfitCode", function()
    ...
    if outfitCode ~= nil then
        ...
        lib.callback("illenium-appearance:server:importOutfitCode", false, function(success)
            if success then
                ShowNotification(_L("outfits.import.success.title"), _L("outfits.import.success.description"), "success")
            else
                ShowNotification(_L("outfits.import.failure.title"), _L("outfits.import.failure.description"), "error")
            end
...
RegisterNetEvent("illenium-appearance:client:generateOutfitCode", function(id)
    lib.callback("illenium-appearance:server:generateOutfitCode", false, function(code)
        if not code then
            ShowNotification(_L("outfits.generate.failure.title"), _L("outfits.generate.failure.description"), "error")
            return
        end
    ...
    if outfitName then
        ...
        lib.callback("illenium-appearance:server:getOutfits", false, function(outfits)
            ...
            if outfitExists then
                ShowNotification(_L("outfits.save.failure.title"), _L("outfits.save.failure.description"), "error")
                return
            end
...
RegisterNetEvent('illenium-appearance:client:updateOutfit', function(outfitID)
    ...
    lib.callback("illenium-appearance:server:getOutfits", false, function(outfits)
        ...
        if not outfitExists then
            ShowNotification(_L("outfits.update.failure.title"), _L("outfits.update.failure.description"), "error")
            return
        end
...
function OpenMenu(isPedMenu, menuType, menuData)
    ...
    if menuType == "default" then
        local costValue = menuData and menuData.customCost or Config.ClothingCost
        local header = string.format(_L("clothing.title"), costValue)
        if isPedMenu then
        ...
        menuItems[#menuItems + 1] = {
            ...
            args = { isPedMenu = isPedMenu, customCost = menuData and menuData.customCost },
            icon = "cart-shopping",
            iconColor = "#ff6b00"
        }
...
RegisterNetEvent("illenium-appearance:client:openClothingShopMenu", function(isPedMenu, customCost)
    if not isPedMenu or type(isPedMenu) == "table" then
        isPedMenu = false
    end
    OpenMenu(isPedMenu, "default", { customCost = customCost })
end)

RegisterNetEvent("illenium-appearance:client:openClothingShop", function(data)
    local isPed, cost = false, nil
    if type(data) == "table" then
        isPed = data.isPedMenu
        cost = data.customCost
    else
        isPed = data
    end
    OpenClothingShop(isPed, cost)
end)
    ...
    if pedModel ~= data.model then
        ...
        lib.callback("illenium-appearance:server:getAppearance", false, function(appearance)
            ...
            if appearance then
                ...
                ShowNotification(_L("outfits.change.failure.title"), _L("outfits.change.failure.description"), "error")
            end
...
RegisterNetEvent("illenium-appearance:client:DeleteManagementOutfit", function(id)
    ...
    ShowNotification(_L("outfits.delete.management.success.title"), _L("outfits.delete.management.success.description"), "success")
end)
...
RegisterNetEvent("illenium-appearance:client:deleteOutfit", function(id)
    ...
    ShowNotification(_L("outfits.delete.success.title"), _L("outfits.delete.success.description"), "success")
end)
...
RegisterNetEvent("illenium-appearance:client:reloadSkin", function(bypassChecks)
    if not bypassChecks and InCooldown() or Framework.CheckPlayerMeta() or cache.vehicle or IsPedFalling(cache.ped) then
        ShowNotification(_L("commands.reloadskin.failure.title"), _L("commands.reloadskin.failure.description"), "error")
        return
    end
...
RegisterNetEvent("illenium-appearance:client:ClearStuckProps", function()
    if InCooldown() or Framework.CheckPlayerMeta() then
        ShowNotification(_L("commands.clearstuckprops.failure.title"), _L("commands.clearstuckprops.failure.description"), "error")
        return
    end
