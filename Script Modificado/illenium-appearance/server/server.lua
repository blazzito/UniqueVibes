...
-- Auto-migration: Ensure 'cost' column exists
CreateThread(function()
    Wait(500)
    local success, result = pcall(MySQL.query.await, "ALTER TABLE appearance_stores ADD COLUMN IF NOT EXISTS cost INT DEFAULT NULL;")
    if success then
        print("[illenium-appearance] [SERVER] [DB] Database migration successful: 'cost' column ensured.")
    else
        -- If IF NOT EXISTS is not supported by older MySQL versions, fallback to a safer check
        MySQL.query("SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME = 'appearance_stores' AND COLUMN_NAME = 'cost'", {}, function(res)
            if not res or #res == 0 then
                MySQL.query("ALTER TABLE appearance_stores ADD COLUMN cost INT DEFAULT NULL")
                print("[illenium-appearance] [SERVER] [DB] Column 'cost' added to appearance_stores table.")
            end
        end)
    end
end)

local function getMoneyForShop(shopType, customCost)
    if customCost and tonumber(customCost) > 0 then
        return tonumber(customCost)
    end
    local money = 0
...
lib.callback.register("illenium-appearance:server:hasMoney", function(source, shopType, customCost)
    local money = getMoneyForShop(shopType, customCost)
...
lib.callback.register("illenium-appearance:server:buyTattooCart", function(source, tattoos, shopDefaultCost)
    local src = source
    local total = 0
    local count = 0
    ...
    if tattoos then
        for _, tattoo in ipairs(tattoos) do
            local price = (tattoo.price and tonumber(tattoo.price) > 0) and tonumber(tattoo.price) or tonumber(shopDefaultCost or 0)
            total = total + price
            count = count + 1
        end
    end
    ...
    if Framework.RemoveMoney(src, "bank", total) then
        lib.notify(src, {
            title = _L("purchase.tattoo.success.title"),
            description = string.format("Has comprado %d tatuajes por $%d", count, total),
            type = "success",
...
lib.callback.register("illenium-appearance:server:payForTattoo", function(source, tattoo, shopType, shopCost)
    -- We now handle bulk purchase in buyTattooCart, but we keep this for backward compatibility or individual clicks if needed
    -- For now, return true to allow preview without instant charge
    return true
end)
...
RegisterServerEvent("illenium-appearance:server:saveAppearance", function(appearance)
    ...
    -- Call the save function (it has its own validation)
    Framework.SaveAppearance(appearance, citizenID)
end)

RegisterServerEvent("illenium-appearance:server:chargeCustomer", function(shopType, customCost)
    local src = source
    if shopType == "tattoo" then return end -- Tattoos are charged per-piece in buyTattooCart
    local money = getMoneyForShop(shopType, customCost)
...
lib.addCommand("fixpj", { help = _L("commands.reloadskin.title") }, function(source)
