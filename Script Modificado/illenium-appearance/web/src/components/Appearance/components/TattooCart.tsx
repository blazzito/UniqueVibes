...
const TattooCart = ({ data, handleDeleteTattoo, handleBuy }: TattooCartProps) => {
  const { locales, tattooPrice, shopType } = useNuiState();
  ...
  const total = appliedTattoos.reduce((acc, tattoo) => {
    const price = tattoo.price !== undefined && tattoo.price > 0 ? tattoo.price : (tattooPrice || 0);
    return acc + price;
  }, 0);
  ...
  return (
    <CartContainer>
          ...
          appliedTattoos.map((tattoo, index) => (
            <CartItem key={`${tattoo.name}-${index}`}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 15 }}>
                {(shopType === 'tattoo' && ((tattoo.price !== undefined && tattoo.price > 0) || tattooPrice > 0)) && (
                  <span style={{ 
                    fontFamily: 'var(--font-bebas)', 
                    color: 'var(--orange-favro)', 
                    fontSize: '1.1rem',
                    fontWeight: 900
                  }}>${tattoo.price !== undefined && tattoo.price > 0 ? tattoo.price : tattooPrice}</span>
                )}
      ...
      <Footer>
        {shopType === 'tattoo' && appliedTattoos.length > 0 && (
          <div style={{ 
            display: 'flex', 
            ...
            <span style={{ color: 'rgba(255,255,255,0.6)', fontFamily: 'var(--font-inter)', fontSize: '0.8rem', fontWeight: 700 }}>TOTAL ACUMULADO</span>
            <span style={{ 
              color: '#fff', 
              fontFamily: 'var(--font-bebas)', 
              fontSize: '1.6rem', 
        ...
        <BuyButton onClick={handleBuy}>
          {shopType === 'tattoo' ? 'COMPRAR TATUAJES' : 'COMPRAR'}
        </BuyButton>
      </Footer>
    </CartContainer>
  );
