...
RegisterNUICallback("appearance_apply_tattoo", function(data, cb)
    local paid = not data.tattoo or not Config.ChargePerTattoo or lib.callback.await("illenium-appearance:server:payForTattoo", false, data.tattoo, CurrentShopType, CurrentShopCost)
    if paid then
...
RegisterNUICallback("appearance_buy_tattoo_cart", function(data, cb)
    local success = lib.callback.await("illenium-appearance:server:buyTattooCart", false, data.tattoos, data.shopDefaultCost)
    cb(success)
end)
...
RegisterNUICallback("appearance_delete_tattoo", function(data, cb)
    ...
    if CurrentShopType == "tattoo" then
        local appearance = client.getAppearance() -- Get the clean snapshot
        appearance.tattoos = data -- Update only the tattoos in the snapshot
        client.updatePlayerAppearance(appearance) -- Commit updated snapshot
...
RegisterNUICallback("rotate_character", function(delta, cb)
    cb(1)
    local ped = cache.ped
    SetEntityHeading(ped, GetEntityHeading(ped) + (delta or 0.0))
end)
