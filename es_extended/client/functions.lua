...
local addonResourcesState = {
    ...
    ['mt-notify'] = GetResourceState('mt-notify') ~= 'missing',
    ['esx_textui'] = GetResourceState('esx_textui') ~= 'missing',
    ['esx_context'] = true, -- redirected to ox_lib
}
...
---@return boolean Success Whether the progress bar was successfully completed or not
...
function ESX.TextUI(text, key, align)
	return exports['mt-notify']:displayTextUI(text, key, align)
end

---@return nil
function ESX.HideUI()
	return exports['mt-notify']:hideTextUI()
end
...
function ESX.ShowHelpNotification(msg, thisFrame, beep, duration)
    exports['mt-notify']:displayTextUI(msg)

    if not thisFrame and duration and duration ~= -1 then
...
function ESX.ShowFloatingHelpNotification(msg, coords)
    exports['mt-notify']:create3DTextUI('floating_help', {
        coords = coords,
        text = msg,
        displayDist = 5.0,
        interactDist = 2.0
...
-- esx_context compatibility shim using ox_lib
local _ctxCallback = nil
local _ctxCloseCallback = nil
local _ctxElements = nil
local _ctxMenuId = 'esx_context_compat'

local function _buildOxOptions(elements, onSelect)
    local options = {}
    for _, el in ipairs(elements) do
        if not el.unselectable and not el.input then
            options[#options+1] = {
                title = el.title or '',
                icon = el.icon,
                onSelect = function()
                    if onSelect then onSelect(nil, el) end
                end
            }
        end
    end
...
function ESX.OpenContext(side, elements, onSelect, onClose)
    _ctxCallback = onSelect
    _ctxCloseCallback = onClose
    _ctxElements = elements
    lib.registerContext({
        id = _ctxMenuId,
        title = (elements and elements[1] and elements[1].title) or 'Menu',
        onExit = function()
            if _ctxCloseCallback then
                _ctxCloseCallback()
            end
        end,
        options = _buildOxOptions(elements, onSelect)
    })
    lib.showContext(_ctxMenuId)
end

function ESX.PreviewContext(...)
    -- no-op: ox_lib has no preview concept, just open
end
...
function ESX.RefreshContext(elements)
    if elements then
        ...
        lib.registerContext({
            ...
            options = _buildOxOptions(elements, _ctxCallback)
        })
        lib.showContext(_ctxMenuId)
    end
end
...
function ESX.UI.ShowInventoryItemNotification(add, item, count)
    exports['mt-notify']:Notify({
        title = "Inventario",
        description = (add and "+ " or "- ") .. count .. "x " .. item,
