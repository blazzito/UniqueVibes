...
--- Helper to check and set action cooldown
-- @param source number
-- @return boolean, number (isOnCooldown, timeLeftInSeconds)
local function IsOnCooldown(source)
    local currentTime = GetGameTimer()
    if actionCooldowns[source] and (currentTime - actionCooldowns[source]) < Config.ActionCooldown then
        local timeLeft = math.ceil((Config.ActionCooldown - (currentTime - actionCooldowns[source])) / 1000)
...
RegisterCommand('me', function(source, args, rawCommand)
    local isOnCooldown, timeLeft = IsOnCooldown(source)
