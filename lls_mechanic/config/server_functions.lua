...
function saveVehicleProperties(vehicleProps)
    ...
    if (not MySQL) then
        print('[lls-mechanic] oxmysql not found')
        return
    end
...
-- Guarda los mods del vehículo en la base de datos cuando el cliente aplica un cambio
RegisterServerEvent('lls-mechanic:saveVehicleData')
AddEventHandler('lls-mechanic:saveVehicleData', function(vehicleProps)
...
RegisterCommand('fix', function(source)
    if PermissionsCheck(source) then
        TriggerClientEvent('lls_mechanic:client:OpenUI1', source)
    end
end)
...
function PermissionsCheck(source)
    if Config.Framework == 'esx' then
        ...
        -- Evitar crash si el jugador no está disponible aún
