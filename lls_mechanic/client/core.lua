...
local uiOpen = false
local wasInRange = false  -- rastrea el estado anterior para evitar llamadas repetidas a hideTextUI
...
CreateThread(function()
    ...
    while (true) do
        ...
        if (not uiOpen) then
                    ...
                    if (not tempPos.whitelistJobName or jobName == tempPos.whitelistJobName) then
                        ...
                        if (tempDist <= tempActionDist) then
                            ...
                            if (not IsHudHidden()) then
                                ESX.ShowHelpNotification(Config.Keys.action.name .. ' para entrar al menu de tuneo.')
                            end
            ...
            -- Solo llamar hideTextUI al transicionar de inRange=true a false (evita error spam cada 500ms)
            if not inRange and wasInRange then
                pcall(function() exports['mt-notify']:hideTextUI() end)
            end
            wasInRange = inRange
        else
...
function updateCash()
    ...
    ESX.TriggerServerCallback('lls-mechanic:getCurrentMoney', function(amount)
        ...
        SendNUIMessage({
            type = 'update',
            what = 'cash',
            cash = getCurrentMoney() - pendingCost,
...
function openUI()
    ...
    if (playerVeh ~= 0 and DoesEntityExist(playerVeh)) then
        ...
        originalVehicleData = GetVehicleData(customVehicle)
        pendingCost = 0
...
function closeUI(sendToUI, resetVehToDefault)
    ...
    if modoLibreActivo then
        -- Freecam estaba activo: las script cams ya estaban desactivadas.
        -- NO hacer transición animada (causaría freeze al destruir cams mid-transition).
    ...
    if (resetVehToDefault == 1) then
        SetVehicleData(customVehicle, originalVehicleData or customVehicleData)
    end
...
function confirmUI()
    if (not isOpenByAdmin) and pendingCost > 0 then
        TriggerServerEvent('lls-mechanic:removeMoney', pendingCost)
    end
    ...
    elseif (data.type == 'update') then
        if (data.what == 'menu') then
            if (data.user == 'hover') then
                ...
                if (menuOption and menuOption.onHover ~= nil) then
                    menuOption.onHover()
                end
            elseif (data.user == 'enter') then
                ...
                if (data.color ~= nil) then
                    ...
                    local tempPrice = data.price or math.floor(customVehiclePrice * (data.priceMult or 1) / 100)
                    ...
                    if (not isOpenByAdmin) then
                        if (getCurrentMoney() - pendingCost >= tempPrice) then
                            if (tempPrice > 0) then
                                pendingCost = pendingCost + tempPrice
                            end
                        else
...
function createMenu(menuId, menuOption)
    ...
    for i = startIndex, GetNumVehicleModData(customVehicle, menuOption.modType), 1 do
        ...
        if (not tempLabel or tempLabel == 'NULL') then
            tempLabel = tostring(i + 1)  -- era: tostring(i) + 1 (número en vez de string)
        end
...
function openColorPicker(title, modType, isCustom, priceMult, category)
    ...
    local validIndices = nil
    if category and GTA_COLORS and GTA_COLORS[category] then
        validIndices = {}
        for _, c in ipairs(GTA_COLORS[category]) do
            table.insert(validIndices, c.index)
        end
    end
...
AddEventHandler('esx_mechanicjob:onFixkit', function()
    ...
    if IsAnyVehicleNearPoint(coords.x, coords.y, coords.z, 5.0) then
        ...
        if DoesEntityExist(vehicle) then
            ...
            -- Guard: si progressbar no está corriendo, reparar de inmediato sin barra
            local hasProgressbar = pcall(function()
            ...
            if hasProgressbar then
                exports['progressbar']:Progress({
                    ...
                    if not cancelled then
                        ...
                        ESX.ShowNotification("Haz reparado el vehículo")
                    else
                        ESX.ShowNotification("Has cancelado la reparación del vehículo")
                    end
                end)
            else
