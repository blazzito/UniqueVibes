...
function SetVehicleData(vehicle, data)
	...
	-- FIX: SET_VEHICLE_MOD_COLOR_1/2 necesitan TODOS sus parámetros (paintType, color, pearlescentColor).
	-- Llamarlos solo con el paintType (como estaba antes) deja "color" y "pearlescentColor" en nil,
	-- lo que provoca que el native falle silenciosamente y el tipo de pintura / nacarado no se aplique.
	-- Por eso se reubica este bloque AL FINAL (después de aplicar color1/color2/pearlescentColor)
	-- y se vuelve a leer el estado ya actualizado del vehículo para no pisar nada.
	if (data.color1Type or data.color2Type) then
		local curColor1, curColor2 = GetVehicleColours(vehicle)
		local curPearlescent = GetVehicleExtraColours(vehicle)

		if (data.color1Type) then
			SetVehicleModColor_1(vehicle, data.color1Type, curColor1, curPearlescent)
		end

		if (data.color2Type) then
			SetVehicleModColor_2(vehicle, data.color2Type, curColor2)
		end
	end
...
function SetVehicleModData(vehicle, modType, data)
	...
	elseif (modType == 'paintType1') then
		-- FIX: el native necesita (vehicle, paintType, color, pearlescentColor). Antes solo se
		-- mandaba el paintType y el resto quedaba en nil -> el cambio de tipo de pintura no se aplicaba.
		local curColor1 = GetVehicleColours(vehicle)
		local curPearlescent = GetVehicleExtraColours(vehicle)
		SetVehicleModColor_1(vehicle, tonumber(data), curColor1, curPearlescent)
	elseif (modType == 'paintType2') then
		-- FIX: el native necesita (vehicle, paintType, color). Antes solo se mandaba el paintType.
		local _, curColor2 = GetVehicleColours(vehicle)
		SetVehicleModColor_2(vehicle, tonumber(data), curColor2)
...
function repairtVehicle(vehicle)
	...
	customVehicleData = GetVehicleData(vehicle)
