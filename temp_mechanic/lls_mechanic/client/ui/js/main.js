...
// Estos slots NO aceptan RGB libre, solo un índice de la lista interna
// del juego (0-159). Los RGB de abajo son solo una aproximación visual
// para que el picker se vea bien; el índice es lo que realmente importa.
// ═══════════════════════════════════════════════════════════════
...
const CARD_OPACITIES = [1, 0.65, 0];    // ±2 invisible, sólo para la transición
...
function applyCardTransforms() {
    ...
    cards.forEach((card, idx) => {
        ...
        if (!t) {
            ...
            // Moverlos fuera de vista sin animación brusca
...
function renderCarousel() {
    ...
    titleEl.textContent = menuTitle || 'MENÚ';
    ...
    options.forEach((opt, idx) => {
        ...
        // Dots de nivel para submenús de mods
        let dotsHtml = '';
        if (isModSub && opt.label !== 'Default') {
            const match = opt.label.match(/^Level\s+(\d+)$/i);
            if (match) {
        ...
        let btnHtml = '';
        if (isCurrent) {
            btnHtml = `<div class="card-btn card-btn-current">✓ SELECCIONADO</div>`;
        } else {
            btnHtml = `<div class="card-btn">Abrir <span class="card-btn-accent">Selección</span> ›</div>`;
        }
    ...
    // Enviar hover a Lua para que el vehículo haga preview
    if (options.length > 0) {
        sendHandle({
...
function navigateTo(idx) {
    if (idx < 0 || idx >= options.length || idx === selectedIndex) return;
    selectedIndex = idx;
    highlightSelected();
    applyCardTransforms();
    sendHandle({
...
function getItemDisplayName(opt) {
    ...
    if (isModSub) {
        if (label === 'Default') {
            ...
            label = label.replace(/^Level\s+(\d+)$/i, (_, n) => `${menuTitle} Nivel ${n}`);
        }
    }
    return label;
}
...
function renderSummary() {
    ...
    confirmBtn.textContent = count > 0 ? '✓  CONFIRMAR TUNEO' : '← SALIR DEL TALLER';
...
function activateItem(idx) {
    ...
    // Registrar en el resumen: solo mods directos (precio explícito, no ya activo, no color — esos se rastrean en cp-ok)
    if (!isColorType && opt.price != null && opt.price !== -1) {
        addToSummary({
            category: menuTitle || 'Modificación',
...
// ═══════════════════════════════════════════════════════════════
// MODO LIBRE — quita foco NUI, cámara de tercera persona normal
// ═══════════════════════════════════════════════════════════════
...
function cpRefresh(sendToLua) {
    ...
    document.getElementById('cp-hex-val').textContent = rgbToHex(r, g, b);
...
function openCp(data) {
    cpModType     = cpModType || cpPendingType || null;
    cpPendingType = null;
    cpTitle       = data.title     || '';
    ...
    if (cpIsCustom) {
        // Color libre (color1 / color2 / neón / humo de llantas)
...
function buildPalette(selectedIdx) {
    cpPaletteIndex = selectedIdx;
    const cont = document.getElementById('cp-palette');
    cont.innerHTML = PALETTE_COLORS.map((c, i) =>
        `<div class="cp-swatch${i === selectedIdx ? ' sel' : ''}" data-i="${i}" style="background:rgb(${c[0]},${c[1]},${c[2]})"></div>`
    ).join('');
...
window.addEventListener('message', ({ data }) => {
    ...
    switch (data.type) {
        ...
        case 'update':
            if (data.what === 'menu') {
                // Fallback: extraer cpModType del menuId si el picker está abierto
                ...
                // Posicionar el foco en el item activo o en defaultOpt
                const currentIdx = options.findIndex(o => o.price === -1);
                selectedIndex = currentIdx >= 0 ? currentIdx : (defaultOpt || 0);
        ...
        case 'freecamEnd':
            // Lua restauró el foco → volvemos al menú normal
            freecamOn = false;
            document.body.style.cursor = '';
            break;
