    ...
    Citizen.CreateThread(function()
        while true do
            ...
            if IsPedSwimmingUnderWater(ped) then
                TriggerEvent('mt_skills:client:AddSkill', 'Swimming', 1)
            elseif GetEntitySpeed(vehicle) * 3.6 >= 100 and GetPedInVehicleSeat(GetVehiclePedIsIn(ped), -1) == ped then
                TriggerEvent('mt_skills:client:AddSkill', 'Driving', 1)
            elseif Config.Skills.AllowOutsideTraining then    
                
                if IsPedRunning(ped) then
                    TriggerEvent('mt_skills:client:AddSkill', 'Stamina', 1)
                end

                if IsPedSprinting(ped) then
                    TriggerEvent('mt_skills:client:AddSkill', 'Running', 1)
                end
            end
            Citizen.Wait(30000)
        end
    ...
    Citizen.CreateThread(function()
        while true do
            ...
            if IsPedShooting(ped) then
                ...
                if GoodWeapon then
                    TriggerEvent('mt_skills:client:AddSkill', 'Shooting', 1)
                end
            end
            Citizen.Wait(200)
        end
