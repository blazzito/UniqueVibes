...
local function hasShopAccess(shop)
	if not PlayerData.loaded then return true end
	return not shop.groups or client.hasGroup(shop.groups)
end

local Shops = {}

function Shops.wipeShops()
	for i = 1, #shops do
		local shop = shops[i]
...
function Shops.refreshShops()
	...
	for type, shop in pairs(shopTypes) do
		...
		if shared.target and (shop.model or shop.targets) then
			if shop.model then
				...
				for i = 1, #shop.targets do
					...
					if target.ped then
						...
						shops[id] = {
							...
							blip = blip and createBlip(blip, target.loc or target.coords)
						}
					end
