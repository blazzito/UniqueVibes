...
local Drawables = {
	...
	["Shoes"] = {
		Drawable = 6,
		Table = {Standalone = true, Male = 16, Female = 99},
...
local Extras = {
	["Shirt"] = {
		Drawable = 11,
		Table = {
			Standalone = true, Male = 15, Female = 140,
			Extra = { 
	...
	["Pants"] = {
		Drawable = 4,
		Table = {Standalone = true, Male = 16, Female = 15},
...
for k,v in pairs(Config.Commands) do
	RegisterCommand(v.name or k, function() 
		v.Func()  
		print(json.encode(LastEquipped))
		local sended = {}
		for k,v in pairs(LastEquipped) do
...
RegisterCommand("check_last", function()
  print("JSON.CLOTHING", json.encode(LastEquipped, { indent = true }))
end, false)

RegisterNUICallback("toggleCloth", function(command, cb)
  print ("Use toggleCloth", command)
	ExecuteCommand(command)

  cb({ succ = true, data = LastEquipped })
end)
