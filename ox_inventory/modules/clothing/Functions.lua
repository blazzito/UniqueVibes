...
function Lang(what)
	local Dict = DPLocale[Config.Language]
	if not Dict[what] then return DPLocale["en"][what] end -- If we cant find a translation, use the english one.
	return Dict[what]
end
...
local oldHairPed, oldHairTex, oldHairColor = nil, nil, nil
local toggle = false


RegisterNetEvent("clothing:toggleHair", function()
    local gender = IsMpPed(PlayerPedId())    
    toggle = not toggle
    if toggle then
        oldHairPed = GetPedDrawableVariation(PlayerPedId(), 2)
        oldHairTex = GetPedTextureVariation(PlayerPedId(), 2)
        oldHairColor = GetPedHairColor(PlayerPedId())
    end

    if toggle then
        if gender == "male" then
            SetPedComponentVariation(PlayerPedId(), 2, 0, oldHairTex, 0)
        elseif gender == "female" then
            SetPedComponentVariation(PlayerPedId(), 2, 0, oldHairTex, 0)
        end
    else
        SetPedComponentVariation(PlayerPedId(), 2, oldHairPed, oldHairTex, 0)
    end
end)
--[[
		[dpc:ToggleMenu]
