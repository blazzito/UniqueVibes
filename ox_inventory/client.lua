	...
	client.tick = SetInterval(function()
		...
		if invOpen then
			...
			if currentWeapon and currentWeapon.timer then
                ...
                -- Manual reload from reserve (Key R)
                if IsControlJustPressed(0, 45) and not invBusy and not usingItem then
                    local clipSize = GetMaxAmmoInClip(playerPed, currentWeapon.hash, true)
                    local _, clipAmmo = GetAmmoInClip(playerPed, currentWeapon.hash)
                    local totalAmmo = GetAmmoInPedWeapon(playerPed, currentWeapon.hash)

                    if clipAmmo < clipSize and totalAmmo > clipAmmo then
                        TaskReloadWeapon(playerPed, true)
                    end
                end
			end
