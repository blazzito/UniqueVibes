let autoEditNewReport = false;

informesFunctions = {
	loadInformesFunctions: () => {
		$(document).on('click', '.police .btn-police-reports', function () {
		...
		$(document).on('click', '.report-type-tab', function () {
			$('.report-type-tab').removeClass('active').css('opacity', '0.5');
			$(this).addClass('active').css('opacity', '1');
			informesFunctions.searchReport();
			if ($('.police ' + policeTabSelected + ' .informes .input-search-report').val() === '') {
				informesFunctions.loadReports();
			}
		});
		...
		$(document).on('click', '.police .informes .add-informe, .police .new-report-button', function () {
		...
		$(document).on('click', '.police .informes .add-vehicle', function (event) {
		...
		$(document).on('click', '.police .btn-add-evidence-url', function () {
			const url = $('.police .input-evidence-url').val();
			if (url && url.trim().length > 0) {
				$('.police ' + policeTabSelected + ' .informes .row.evidences').find('.empty-collection').remove();
				$('.police ' + policeTabSelected + ' .informes .row.evidences').append(`
					<div class="col-4 pt-2 scale-in">
						<div class="evidence">
							<img src="${url}" style="filter:hue-rotate(-${currentHueDeg}deg);">
							<button class="btn text-white p-0 mt-2 delete-evidence"><i class="lni lni-trash-can"></i></button>
						</div>
					</div>
				`);
				$('.police .evidencias-container .personas-box').removeClass('scale-in').addClass('scale-out');
				$('.police .evidencias-container').fadeOut(300);
				$('.police .input-evidence-url').val('');
				informesFunctions.updateEvidencias();
			} else {
				sendNotification('error', Translations.NoImageUrl || 'Please enter a valid URL');
			}
		});
		...
		$(document).on('change', '.police .informes .select-tags', function () {
			...
			if (!$('.police .informe-report').hasClass('editing')) {
				$(this).val('0');
				return sendNotification('error', Translations.EditReport);
			}
			const val = $(this).val();
			if (val) {
				$('.police ' + policeTabSelected + ' .informes .tag-list .empty-collection').remove();
	...
	loadReports: () => {
		...
		$('.police ' + policeTabSelected + ' .informes .title-2 .btn-edit-report').hide();
		$('.police ' + policeTabSelected + ' .informes .title-2 .btn-finish-report').hide();

		const hasDivision = (divId) => {
			if (!jobData || !jobData.divisions) return false;
			const currentDivs = jobData.divisions;
			if (Array.isArray(currentDivs)) {
				return currentDivs.includes(divId);
			} else {
				return currentDivs[divId] === true || currentDivs[divId] == "true" || currentDivs[divId] == 1;
			}
		};
		$('.police ' + policeTabSelected + ' .informes .report-list')
			.html('')
			.fadeOut(0, function () {
				const currentType = $('.police ' + policeTabSelected + ' .report-type-tab.active').data('type') || 'informe';
				TriggerCallback('origen_police:police:Get100Reports', { type: currentType }).done((cb) => {
					policeFunctions.setServiceBadge('reports', cb && cb.length ? cb.length : 0);
					if (cb && cb.length > 0) {
		...
		const canManageAllDivisionTags = permissions[jobData.name] && jobData.level >= permissions[jobData.name]["ManageAllDivisionTags"];

		if (tags && (tags[jobData.name] || tags['default'])) {
			const jobTags = tags[jobData.name] != null ? tags[jobData.name] : tags['default'];
			jobTags.map((tag) => {
				let isDivisionTag = false;
				let playerHasDivision = false;
				if (divisions) {
					Object.values(divisions).forEach(div => {
						if (tag && div.name && ("Division: " + div.name).trim() == tag.trim()) {
							isDivisionTag = true;
							if (hasDivision(div.id)) {
								playerHasDivision = true;
							}
						}
					});
				}

				if (!isDivisionTag || playerHasDivision || canManageAllDivisionTags) {
			...
			function () {
				...
				if (text.length > 0 || tag) {
					data = {
						text,
						tags: false,
						type: $('.police ' + policeTabSelected + ' .report-type-tab.active').data('type') || 'informe'
					};
					if (tag) {
						data.tags = tag;
					}
	...
	loadInformeTags: () => {
		...
		if (!tags || !tags[jobData.name]) return;
	...
	addMultaInforme: function (articulos, importe, meses, cid) {
		...
		informesFunctions.updatePersonasInvolucradas(() => {
			if (currentReport != null) informesFunctions.loadInforme(currentReport);
		});
	},

	updatePersonasInvolucradas: function (cbFunc) {
		let data = {
			key: 'implicated',
			value: []
		};
		...
		informesFunctions.updateInforme(data, cbFunc);
	},
		...
		TriggerCallback('origen_police:police:NewReport', {}).done((cb) => {
			if (cb) {
				...
				const findAndClick = setInterval(() => {
					const el = $(`.police .report-list .report#report-${cb.id}`);
					if (el.length > 0) {
						el.trigger('click');
						clearInterval(findAndClick);
					}
					attempts++;
					if (attempts > 50) clearInterval(findAndClick);
				}, 100);
			}
		});
	},
	...
	addVehicleInforme: function (plate, name) {
		if (
			...
			$('.police ' + policeTabSelected + ' .informes .vehiculos-list .no-notes').remove();
		}
			...
			if (cb) {
				...
				$('.police .informes .informe-report .content-report-' + reportid)
                    ...
                    <div class="empty-state">
                        ...
                        <div class="empty-state-text">${Translations.SelectReport}</div>
                    </div>
                `
							)
							.fadeIn(300);
	...
	updateInforme: function (data, cbFunc) {
			...
			function () {
				TriggerCallback('origen_police:police:GetReport', { reportid }).done(
                    ...
                    `;
						...
						} else if (cb.tags && String(cb.tags).trim() !== '' && String(cb.tags).toLowerCase() !== 'null' && String(cb.tags).toLowerCase() !== 'undefined') {
							tags = `
                        <div class="tag scale-in">
                            <span>${cb.tags}</span>
                            <i class="fas fa-times"></i>
                    ...
                    `;
						...
						if (!tags) {
							tags = `
                        <div class="empty-collection scale-in">
                    ...
                    `;
                        ...
                        <div class="col-4 pt-2">
                            <div class="evidence">
                                ...
                                <button class="btn text-white p-0 mt-2 delete-evidence" style="display:none"><i class="lni lni-trash-can"></i></button>
                            </div>
                        </div>
                    ...
                    `;
						...
						if (cb.implicated && isJsonString(cb.implicated)) {
							...
							if (implicatedList.length > 0) {
								implicatedList.forEach((imp) => {
									...
									implicated += `
                            <li class="list-group-item list-group-item-action scale-in" data-id="${imp.citizenid}">
                                <div class="d-flex align-items-center">
                                    <h5 class="persona-name">${imp.name}</h5>
                                    <div class="ms-2 text-muted persona-cid"><i class="fas fa-id-card me-1"></i> ${imp.citizenid}</div>
                                </div>
                                ...
                                <div class="row multas-informes ${classBill}" billid="${billid}">
                                    ...
                                    <div class="col-6">
                                        <div class="info-box h-100">
                                            <div class="info-box-title">${Translations.TotalPenalty}</div>
                                            <div class="info-box-value text-danger">${imp.bills.months ? imp.bills.months + ' ' + Translations.Month : '0 ' + Translations.Month}</div>
                                        </div>
                                    </div>
                                    <div class="col-6">
                                        <div class="info-box h-100">
                                            <div class="info-box-title">${Translations.TotalAmount}</div>
                                            <div class="info-box-value text-success">${imp.bills.price ? imp.bills.price + '$' : '0$'}</div>
                                        </div>
                                    </div>
                                ...
                                <div class="multa-button" style="display:none">
                                    <i class="fas fa-edit"></i> ${Translations.SendFine}
                                </div>
                            </li>
                        `;
						...
						$('.police .informes .title-2 .btn-finish-report').removeClass('btn-finish-report').addClass('btn-edit-report').html(`<i class="fas fa-edit"></i> <span>${Translations.EditReport || 'Editar Informe'}</span>`);
						$('.police ' + policeTabSelected + ' .informes .title-2 .btn-edit-report').show();
                ...
                <div class="row m-titles content-report-${cb.id}">
                	<div class="col-4 p-0">
                        <div class="info-box m-1 h-100">
                            ...
                            <div class="info-box-value">
                                <input type="text" class="input-report-name" value="${cb.title}" readonly>
                            </div>
                    ...
                    <div class="col-6 mt-2 p-0">
                        <div class="info-box m-1 h-100">
                            ...
                            <div class="info-box-value">
                                <input type="text" class="input-report-ubi" value="${cb.location}" readonly>
                            </div>
                        </div>
                    </div>
                </div>
						<div class="row m-titles mt-3">
							<div class="col-12 p-0">
								<div class="info-box m-1 mt-2">
									...
									<div class="citizen-info-container-mini mt-2 d-flex flex-wrap citizen-informes align-content-start">
										<textarea class="input w-100 input-report-desc" placeholder="${Translations.EnterReportDesc}" rows="7" readonly>${cb.description}</textarea>
									</div>
								</div>
							</div>
							<div class="col-6 p-0">
								<div class="info-box m-1 mt-2">
									<div class="notes-title d-flex justify-content-between align-items-center">
										...
										<div class="new-button add-prueba" style="display:none"><i class="fas fa-plus"></i> ${Translations.AddEvidence}</div>
									</div>
							...
							<div class="col-6 p-0">
								<div class="info-box m-1 mt-2">
									<div class="d-flex justify-content-between align-items-center">
										...
										<div class="new-button add-persona" style="display:none"><i class="fas fa-plus"></i> ${Translations.AddPeople}</div>
									</div>
									<div class="citizen-info-container mt-2">
							...
							<div class="col-6 p-0">
								<div class="info-box m-1 mt-2">
									<div class="d-flex justify-content-between align-items-center">
										...
										<div class="new-button add-agente" style="display:none"><i class="fas fa-plus"></i> ${Translations.AddAgent}</div>
									</div>
							...
							<div class="col-6 p-0">
								<div class="info-box m-1 mt-2">
									<div class="d-flex justify-content-between align-items-center">
										...
										<select class="input select-tags p-0" disabled style="display:none">
							...
							<div class="col-6 p-0">
								<div class="info-box m-1 mt-2">
									<div class="d-flex justify-content-between align-items-center">
										...
										<div class="new-button add-victima" style="display:none"><i class="fas fa-plus"></i> ${Translations.AddVictim}</div>
							...
							<div class="col-6 p-0">
								<div class="info-box m-1 mt-2">
									<div class="d-flex justify-content-between align-items-center">
										...
										<div class="new-button add-vehicle" style="display:none"><i class="fas fa-plus"></i> ${Translations.AddVehicle}</div>
							...
							<div class="col-12 text-center">
								<button class="btn btn-danger delete-report mt-3 mb-2" style="display:none"><i class="lni lni-trash-can"></i> ${Translations.DestroyReport}</button>
							</div>
						</div>
					`
							)
						...
						if (autoEditNewReport) {
							autoEditNewReport = false;
							setTimeout(() => {
								$('.police ' + policeTabSelected + ' .informes .title-2 .btn-edit-report:visible').trigger('click');
							}, 120);
						}
					}
				);
