agentesFunctions = {
	activeJobFilter: 'all',
	applyAgentsFilters: () => {
		const tabSelector = '.police ' + policeTabSelected + ' .agentes';
		const searchText = ($(tabSelector + ' .input-search-agent').val() || '').toLowerCase().trim();
		const activeFilter = (agentesFunctions.activeJobFilter || 'all').toLowerCase();

		$(tabSelector + ' .agent-list .agents-items .agent').each(function () {
			const $agent = $(this);
			const agentText = ($agent.text() || '').toLowerCase();
			const agentJob = (($agent.attr('job') || '').toLowerCase());
			const matchesText = searchText.length <= 1 || agentText.includes(searchText);
			const matchesJob = activeFilter === 'all' || agentJob === activeFilter;

			if (matchesText && matchesJob) {
				$agent.stop(true, true).fadeIn(150);
			} else {
				$agent.stop(true, true).fadeOut(150);
			}
		});
	},
	loadAgentesEvents: () => {
		...
		$(document).on('click', '.police .agent-grade', function () {
			...
			fetch('GetAllCategoryGrades', {}).then((data) => {
				let modalContent = '';
				const citizenid = $(
				...
				for (const jobName in data) {
					...
					for (const gradeKey in jobData.grades) {
						const grade = jobData.grades[gradeKey];
						const gradeType = grade.type || jobData.label;

						if (gradesByType[gradeType] == null) {
							gradesByType[gradeType] = {};
						}
						gradesByType[gradeType][gradeKey] = grade;
					}

					
					for (let typeName in gradesByType) {
						let gradeContent = '';
						for (const gradeKey in gradesByType[typeName]) {
							const d = gradesByType[typeName][gradeKey];
							
							gradeContent += `
							<div class="grade" data-id="${gradeKey}" onclick="agentesFunctions.setRange('${jobName}', '${citizenid}', ${gradeKey}, '${d.label || d.name}')">
						...
						modalContent += `<div class="col-6" style="width:${100 / Object.keys(data).length}%">
							<div class="title-1-menu">${typeName.toUpperCase()}</div>
							${gradeContent}
						</div>`
					}
				}
		...
		$(document).on('click', '.police .police-action-btn.add-agent-button', function () {
				...
				if (cb && cb.length > 0) {
					cb.map((citizen) => {
                        ...
                        <div class="col-4 h-100 mb-3">
                            <div class="citizen-box" onclick="agentesFunctions.contratarAgenteModal('${citizen.citizenid
							}')">
                                <div class="citizen-image image-${citizen.citizenid
							}" style="background-image:url(${citizen.image || defaultImage
							}); filter:hue-rotate(-${currentHueDeg}deg)"></div>
                                <div class="p-2 text-center">
                                    <div class="citizen-name">${citizen.firstname + ' ' + citizen.lastname
							}</div>
		...
		$(document).on('click', '.police .btn-search-agent', function () {
			agentesFunctions.applyAgentsFilters();
		});

		$(document).on('keyup', '.police .input-search-agent', function () {
			agentesFunctions.applyAgentsFilters();
		});

		$(document).on('click', '.police .agent-filter-btn', function () {
			const filter = ($(this).attr('data-job') || 'all').toLowerCase();
			agentesFunctions.activeJobFilter = filter;
			$('.police ' + policeTabSelected + ' .agentes .agent-filter-btn').removeClass('active');
				...
				TriggerCallback('origen_police:police:GetPoliceList', {}).done((cb) => {
					if (cb && cb.length > 0) {
						...
						cb.sort(function (a, b) {
							const byJob = a.job.localeCompare(b.job);
						...
						const jobsMap = {};
						cb.forEach((agente) => {
							if (!jobsMap[agente.job]) {
								jobsMap[agente.job] = (agente.titleLabel || agente.job || '').toUpperCase();
							}
						});

						const rawAllLabel = Translations.All || Translations.AllTags || 'TODOS';
						const allLabel = (rawAllLabel + '').toUpperCase().includes('MISSING KEY') ? 'TODOS' : rawAllLabel;
						let filtersHtml = `
							<button class="agent-filter-btn active" data-job="all">${allLabel}</button>
						`;
						Object.entries(jobsMap).forEach(([jobName, label]) => {
							filtersHtml += `<button class="agent-filter-btn" data-job="${jobName}">${label}</button>`;
						});

						$(policeTabSelected + ' .agent-list').append(`
							<div class="agents-controls">
								<div class="agent-filters">${filtersHtml}</div>
								<div class="row m-titles search-box">
									<div class="col-10 p-0">
										<input
											...
											class="search-input w-100 input-search-agent" />
									</div>
									<div class="col-2 p-0">
						...
						cb.map((agente) => {
							...
							$(policeTabSelected + ' .agent-list .agents-items').append(`
								<div class="white-block agent agent-card agente-${citizenId}" job="${citizenJob}" cid="${citizenId}">
						...
						agentesFunctions.activeJobFilter = 'all';
						agentesFunctions.applyAgentsFilters();
					} else {
		...
		$('.police ' + policeTabSelected + ' .agentes .agent-ficha').fadeOut(
			...
			function () {
				TriggerCallback('origen_police:police:GetPolice', {
					...
					agentesFunctions.currentAgent = cb;
                        ...
                        <div class="d-flex w-100 flex-data">
                            ...
                            <div class="w-33">
                                <div class="info-box m-1">
                                    ...
                                    <div class="info-box-value text-uppercase jurisdiction">${cb.department
							}</div>
                ...
                <div class="row mt-2">
                    ...
                    <div class="col-12 text-center">
                        <button class="btn btn-danger mt-3 mb-2" onclick='agentesFunctions.modalDespedirAgente("${cb.citizenid
	...
	searchAgente: (text) => {
		const input = $('.police ' + policeTabSelected + ' .agentes .input-search-agent');
	...
	addCondecorate: () => {
		...
		Object.values(condecorates).forEach((condecoracion) => {
			if (agentesFunctions.currentAgent && agentesFunctions.currentAgent.condecorates && agentesFunctions.currentAgent.condecorates[condecoracion.id]) {
				return;
			}
			if (condecoracion.category == undefined || (jobData && jobData.category && condecoracion.category == jobData.category)) {
				condecoraciones += `
				<div class="col-4">
	...
	setCondecorate: (id, url, name) => {
		...
		TriggerCallback('origen_police:police:UpdatePoliceMetaData', {
			...
			if (cb) {
				if (!agentesFunctions.currentAgent) agentesFunctions.currentAgent = {};
				if (!agentesFunctions.currentAgent.condecorates) agentesFunctions.currentAgent.condecorates = {};
				agentesFunctions.currentAgent.condecorates[id] = true;
				$(
					'.police ' +
					policeTabSelected +
	...
	removeCondecorate: (id, yo) => {
		...
		TriggerCallback('origen_police:police:UpdatePoliceMetaData', {
			...
			if (cb) {
				if (agentesFunctions.currentAgent && agentesFunctions.currentAgent.condecorates) delete agentesFunctions.currentAgent.condecorates[id];
	...
	addDivision: () => {
		...
		Object.values(divisions).forEach((division) => {
			if (agentesFunctions.currentAgent && agentesFunctions.currentAgent.divisions && agentesFunctions.currentAgent.divisions[division.id]) {
				return;
			}
			if (division.category == undefined || (jobData && jobData.category && division.category == jobData.category)) {
				htmlDivisions += `
				<div class="col-4">
	...
	setDivision: (id, url, name) => {
		...
		TriggerCallback('origen_police:police:UpdatePoliceMetaData', {
			...
			if (cb) {
				if (!agentesFunctions.currentAgent) agentesFunctions.currentAgent = {};
				if (!agentesFunctions.currentAgent.divisions) agentesFunctions.currentAgent.divisions = {};
				agentesFunctions.currentAgent.divisions[id] = true;
				$(
					'.police ' +
					policeTabSelected +
	...
	removeDivision: (id, yo) => {
		...
		TriggerCallback('origen_police:police:UpdatePoliceMetaData', {
			...
			if (cb) {
				if (agentesFunctions.currentAgent && agentesFunctions.currentAgent.divisions) delete agentesFunctions.currentAgent.divisions[id];
				yo.addClass('scale-out').fadeOut(500, function () {
					yo.remove();
				});
			}
