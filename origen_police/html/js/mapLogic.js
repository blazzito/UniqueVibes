...
var HomeAlertsGroup = L.layerGroup();

var Icons = {
	Central: CentralGroup,
	Home: HomeAlertsGroup
};
...
var currentLayer
var heatLayer = null;
let shapesDrawInitialized = false;
	...
	mapId.on('draw:created', function (e) {
		...
		if (editableDrawings[type] && editableDrawings[type].name) {
			...
			OpenModal(
				...
				<div class="row">
					<div class="col-12">
						<input class="form-control w-200 map-new-point mb-2" placeholder="${Translations.Title}">
						<select class="form-select w-200 map-new-color mt-3" style="background-color: #1a1a1a; color: white; border: 1px solid #333;">
							<option value="#3388ff" selected style="color: #3388ff;">Azul (Por defecto)</option>
							<option value="#dc3545" style="color: #dc3545;">Rojo</option>
							<option value="#28a745" style="color: #28a745;">Verde</option>
							<option value="#000000" style="color: #000000;">Negro</option>
							<option value="#d63384" style="color: #d63384;">Rosa</option>
						</select>
					</div>
				</div>
				`,
				`<button class="btn-modal" onclick="centralFunctions.createShape($('.map-new-point').val(), $('.map-new-color').val())">${Translations.Save}</button>`,
				Translations.Cancel
			);
		}
	});
...
function GetDataShapes(data) {
	...
	if (data.data && data.data.color) {
		options.color = data.data.color;
		options.fillColor = data.data.color;
	}

	switch (data.type) {
		case 'marker':
			if (data.data && data.data.color) {
				let iconHtml = `<i class="fa-solid fa-location-dot" style="color: ${data.data.color}; font-size: 3vh; text-shadow: 0 0 5px rgba(0,0,0,0.5);"></i>`;
				layer = L.marker(data.data.coordinates.reverse(), { icon: L.divIcon({ html: iconHtml, className: 'custom-colored-marker', iconSize: [30, 30], iconAnchor: [15, 30] }) });
			} else {
				layer = L.marker(data.data.coordinates.reverse());
			}
			break;
		case 'circle':
			layer = L.circle(data.data.coordinates.reverse(), { radius: data.radius, ...options });
			break;
		case 'rectangle':
			layer = L.rectangle(data.data.coordinates, options);
			break;
		case 'polygon':
			layer = L.polygon(data.data.coordinates, options);
			break;
		case 'polyline':
			layer = L.polyline(data.data.coordinates, options);
			break;
		default:
...
function loadShapesMap() {
	...
	setTimeout(() => {
		...
		if (!shapesDrawInitialized) {
			loadDrawPlugins(mapShapes);
			shapesDrawInitialized = true;
		}

		setTimeout(() => {
			if (mapShapes && $('.police .operations .leaflet-draw-toolbar').length === 0) {
				loadDrawPlugins(mapShapes);
			}
		}, 300);
	}, 500);
...
function addBlipAtCoords(id, lat, lng, title, street, time, index) {
	...
	let isEmergency = title && title.toUpperCase().includes('QRR');
...
function CreateBlip(map, blipid, coords, blip, name) {
	if (Blips[blipid] || (typeof BlipsHome !== 'undefined' && BlipsHome[blipid])) {
		mapFunctions.updateBlip(blipid, coords, blip, name);
		return;
	}

	let icondataCentral = L.icon({
		iconUrl: blip,
		iconSize: [20, 20],
		iconAnchor: [10, 20],
		popupAnchor: [0, -10]
	...
	if (typeof mapCentral !== 'undefined' && mapCentral) {
		Blips[blipid] = L.marker([coords.y, coords.x], { icon: icondataCentral })
			.addTo(mapCentral)
			.bindPopup('<b>' + name + '</b>');
	}

	if (typeof mapHome !== 'undefined' && mapHome) {
		BlipsHome[blipid] = L.marker([coords.y, coords.x], { icon: icondataHome })
			.addTo(mapHome)
			.bindPopup('<b>' + name + '</b>');
	}
}
...
function CreateReferenceBlip(map, blipid, coords, blip, name, color) {
	...
	let icondataHome = L.divIcon({
		iconSize: [8, 8],
		iconAnchor: [4, 4],
		popupAnchor: [4, -4],
		html: `<div class="reference-blip home-blip" style="background-color: ${color}"><img src="${blip}"></div>`
	});

	if (typeof mapCentral !== 'undefined' && mapCentral) {
		Blips[blipid] = L.marker([coords.y + 5, coords.x + 5], { icon: icondataCentral })
			.addTo(mapCentral)
			.bindPopup('<b>' + name + '</b>');
	}

	if (typeof mapHome !== 'undefined' && mapHome) {
		BlipsHome[blipid] = L.marker([coords.y + 5, coords.x + 5], { icon: icondataHome })
			.addTo(mapHome)
			.bindPopup('<b>' + name + '</b>');
	}
}
...
const mapFunctions = {
	...
	destroyBlip: (blipid) => {
		...
		if (typeof BlipsHome !== 'undefined' && BlipsHome[blipid]) {
			if (typeof mapHome !== 'undefined' && mapHome) {
				mapHome.removeLayer(BlipsHome[blipid]);
			}
			delete BlipsHome[blipid];
		}
	},
	...
	updateBlip: (blipid, coords, blip, name) => {
		...
		if (typeof mapCentral !== 'undefined' && mapCentral) {
			if (Blips[blipid]) {
				...
				Blips[blipid].setIcon(icondataCentral);
		...
		if (typeof mapHome !== 'undefined' && mapHome) {
			if (typeof BlipsHome !== 'undefined' && BlipsHome[blipid]) {
				BlipsHome[blipid].setLatLng([coords.y, coords.x]);
				BlipsHome[blipid].setIcon(icondataHome);
				BlipsHome[blipid].bindPopup('<b>' + name + '</b>');
			} else {
				BlipsHome[blipid] = L.marker([coords.y, coords.x], { icon: icondataHome })
					.addTo(mapHome)
					.bindPopup('<b>' + name + '</b>');
	...
	updateBlipRef: (blipid, coords, blip, name, color) => {
		...
		if (typeof mapHome !== 'undefined' && mapHome) {
			if (typeof BlipsHome !== 'undefined' && BlipsHome[blipid]) {
				BlipsHome[blipid].setLatLng([coords.y + 5, coords.x - 5]);
				BlipsHome[blipid].setIcon(icondataHome);
				BlipsHome[blipid].bindPopup('<b>' + name + '</b>');
			} else {
				BlipsHome[blipid] = L.marker([coords.y + 5, coords.x - 5], { icon: icondataHome })
...
function initHomeMap() {
	...
	setTimeout(() => {
		if (mapHome) {
			...
			if (typeof alertsFunctions !== 'undefined' && alertsFunctions.renderAlerts) {
