...
settingsFunctions = {
                    ...
                    <li class="list-group-item list-group-item-action">
                        <div class="d-flex justify-content-between align-items-center">
                            <div class="d-flex align-items-center marker-option">
                                ...
                                <div class="marker-btn" onclick="settingsFunctions.createMarker('${marker.name}', ${GetMarkerData(marker.name).fields.length > 1})">
    ...
    gotoMarker: function (x, y, z) {
        fetch(`https://${GetParentResourceName()}/GotoAdminMarker`, {
            method: 'POST',
    ...
    loadStation: function (station) {
        ...
        TriggerCallback('origen_police:callback:GetStation', { station: station }).done((cb) => {
            if (cb) {
                ...
                if (cb && cb.markers) {
                    fetch(`https://${GetParentResourceName()}/UpdateAdminMarkers`, {
                        method: 'POST',
                        body: JSON.stringify(cb.markers)
                    });
                }
                if (cb && cb.markers) {
    ...
    updateMarkerPosition: function (markerName, markerIndex, multiple) {
        if (!currentStation) return;
        fetch(`https://${GetParentResourceName()}/SetAdminMarkerPos`, {
            method: 'POST',
            body: JSON.stringify({ station: currentStation, markerName: markerName, markerIndex: markerIndex + 1, multiple: multiple })
        }).then(() => {
            closeMenu();
        });
    },
    ...
    createMarker: function (markerName, multiple) {
        if (!currentStation) {
            sendNotification('error', "Station required", "You must select a station first to add a marker.");
            return;
        }
        fetch(`https://${GetParentResourceName()}/CreateAdminMarker`, {
            method: 'POST',
            body: JSON.stringify({ station: currentStation, markerName: markerName, multiple: multiple })
        }).then(() => {
            closeMenu();
        });
    },
    ...
    createStation: function () {
        Swal.fire({
            ...
            html:
                '<input id="swal-input1" class="swal2-input" placeholder="Station ID (e.g. vinewood)">' +
                '<input id="swal-input2" class="swal2-input" placeholder="Station Label (e.g. Vinewood PD)">',
            focusConfirm: false,
            ...
            if (result.isConfirmed) {
                ...
                const label = result.value[1];
                if (!name || !label) {
                    sendNotification('error', "Error", "Both fields are required.");
                    return;
                }
                fetch(`https://${GetParentResourceName()}/CreateStation`, {
                    method: 'POST',
                    body: JSON.stringify({
                        name: name,
                        label: label
