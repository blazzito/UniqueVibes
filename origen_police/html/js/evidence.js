async function AnalizeEvidences(list, _date) {
	...
	list.map((l) => {
		...
		if (l.adn) {
			...
			} else if (l.type == 'FinguerPrint' || l.type == 'FingerPrint') {
				tag = 'CitizenID';
			}
			datos.push(
		...
		if (l.color) {
			datos.push(
				`<div class="dato" style="display: flex;"><div class="black underline">${Translations.Color}:</div> <div style="width:5vh;margin-left:10px;background-color:rgb(${l.color[0] + ',' + l.color[1] + ',' + l.color[2]
				})"></div></div>`
			);
		}
		if (l.shooted) {
		...
		if (l.time) {
			...
			datos.push(
				`<div class="dato"><div class="black underline">${Translations.ApproximateTime}:</div> <div>${fecha} - ${hora + ':' + minutos
				}</div></div>`
			);
		}
