local ApartmentStructure = {
    ...
    ["origen_housing"] = {
        func = function(citizenid)
            local houses
            pcall(function()
                if exports.origen_housing.getOwnedHouses then
                    houses = exports.origen_housing:getOwnedHouses(citizenid)
                elseif exports.origen_housing.getPlayerHouses then
                    houses = exports.origen_housing:getPlayerHouses(citizenid)
                end
            end)
            local result = {}
            if houses then
                for _, houseID in pairs(houses) do
                    local house
                    pcall(function()
                        house = exports['origen_housing']:getHouse(houseID)
                    end)
                    if type(house) == 'table' then
                        table.insert(result, { label = house.name or house.label or ("House " .. houseID) })
                    end
                end
            end
            return result
