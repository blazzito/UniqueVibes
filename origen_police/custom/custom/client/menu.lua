...
function OpenMenu(...)
    ...
    if Config.Framework == "qbcore" then
        if menuType == 'dialog' then
            local title = select(4, ...).title
            local onSubmit = select(5, ...)
            print("^5[Menu Debug]^0 Preparing Dialog with title:", title)
            CreateThread(function()
                print("^5[Menu Debug]^0 Calling org-input...")
                local input = exports['org-input']:ShowInput({
                    header = title,
                    inputs = {
                        {
                            name = "value",
                ...
                print("^5[Menu Debug]^0 org-input returned:", json.encode(input))
                if input then
                    local data = {}
                    data.value = input.value or input[1] or input
                    print("^5[Menu Debug]^0 Shooting onSubmit with value:", data.value)
                    onSubmit(data, {close = function() end})
                else
                    print("^5[Menu Debug]^0 org-input was cancelled or returned nil")
                end
            end)
        else
            exports["org-menu"]:openMenu(...)
        end
    elseif Config.Framework == "esx" then
...
function ProgressBar(name, label, duration, useWhileDead, canCancel, disableControls, animation, prop, propTwo, onFinish, onCancel)
    ...
    elseif Config.Framework == "esx" then
        if GetResourceState("esx_progressbar") ~= "missing" then
            ...
            Framework.Progressbar(label, duration, { -- This function is already async
                FreezePlayer = false
            })
            if animation then
            ...
            if time < 2 then
                -- This means that the ProgressBar function is creating a thread
                Wait(duration)
            end
            if onFinish then
                onFinish()
